...
void multigrid_d3::mgSolve(sfield &inFn, const sfield &rhs) {
    pressureData = 0.0;
    residualData = 0.0;
    inputRHSData = 0.0;

    // TRANSFER DATA FROM THE INPUT SCALAR FIELD INTO THE DATA-STRUCTURES USED BY poisson
    inputRHSData(stagCore) = rhs.F.F(stagCore);
...
void multigrid_d3::createMGSubArrays() {
    ...
    for(int i=0; i<=inputParams.vcDepth; i++) {
        ...
        MPI_Type_indexed(numPoints, blockSize.data(), blockIndx.data(), MPI_DOUBLE, &xMGArray(i));
        ...
        MPI_Type_indexed(numPoints, blockSize.data(), blockIndx.data(), MPI_DOUBLE, &yMGArray(i));
